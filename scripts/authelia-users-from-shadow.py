...
def write_users_yaml(users: dict):
    def inner():
        with open('/var/lib/authelia/users_database.yml', 'x') as f:
            yaml.dump(users, f)

    try: inner()
    except FileExistsError as e: 
        ts = datetime.now().isoformat()
        print(f'Renaming existing conf to /var/lib/authelia/users_database-{ts}.yml')
        os.rename('/var/lib/authelia/users_database.yml', f'/var/lib/authelia/users_database-{ts}.yml')
        inner()
